...
async function postPayment(val, callback) {
    ...
    .send(JSON.stringify({"account":val.account,"accountid":val.accountid,"type":val.type,"method":"MOMO","netWork":val.network,"mobile":val.mobile,"source":"USSD","amount":val.groupid,"amount":val.amount,"reference":"Group Save","tenantid":tenant,"withdrawal":val.withdrawal}))
    .end( async(res)=> { 
        if (res.error) throw new Error(res.error); 
        console.log(res.raw_body);
        var response = JSON.parse(resp.raw_body);
